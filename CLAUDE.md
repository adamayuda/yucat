...
- **recipes**: Recipes tab — searchable, category-filtered list of home-made cat treats. **UI-first: the screen defines the shape the backend will fill.** `RecipeSeedDataSource` (`data/datasources/`) is a hard-coded catalogue and is **the single swap point** for a real remote source. Two things to know: recipe copy (`name`, `description`) is **canonical English** like `ProductEntity`'s — the backend will carry translations, so it deliberately does not go through the ARBs; and `compatibility` is a **seeded flag**, not computed — the per-cat rules land with the backend. Filtering is in-memory and **undebounced on purpose** (unlike `SearchBloc`, which debounces because it hits Algolia). Tapping a card pushes `RecipeDetailRoute` — a **stateless, bloc-free** screen (hero, meta, ingredients, numbered steps, optional tip); everything it renders arrives on the `RecipeDisplayModel` the route carries, so there's no async work to orchestrate
