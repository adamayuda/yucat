...
8. UseCases — search/brand, log-event, fetch-product-by-image, cat CRUD, auth, subscription, saved-products (get/isSaved/save/unsave), saved-litters (same four), scan-history, litter-history, get-recipes
...
10. BLoCs (**17**, registered as factories) — Splash, OnBoarding, Search, Home, Profile, Recipes, FoodGuide, Articles, ProductListing, ProductDetail, LitterDetail, CatListing, CatDetail, CatCreate, Paywall, SavedProducts, ScanHistory. ⚠️ `FoodGuideBloc` and `ArticlesBloc` are the **second and third** blocs deliberately absent from `main.dart`'s `MultiBlocProvider` (after `CatCreateBloc`) — Home's `FoodGuideSection` and `HomeNewsCard` are their only consumers and own the instances
