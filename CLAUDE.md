...
npx firebase-tools@latest deploy --only functions:fetchProductByImageV2,functions:analyzeProductLabel,functions:nightlySelfHeal,functions:generateCatNarrative,functions:analyzeBrand
...
- **Tests**: `flutter test` runs the unit tests under `test/` — currently only the **pure, Flutter-free** parts of the health carnet (`cat_health_schedule.dart`, `health_calendar_layout.dart`, `cat_allergen.dart`), which is why those files import no Flutter and take `now` as a parameter. There is no `test` package, so tests import `flutter_test` and must run with `flutter test`, not `dart test`. `integration_test/onboarding_screenshots_test.dart` is the only integration test. Nothing else in the app is covered
