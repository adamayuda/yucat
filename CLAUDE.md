...
- **articles**: Editorial cat-care articles. Backed by the Firestore **`articles`** collection (doc id = authored slug), on the **same contract as recipes and the food guide** — canonical English in flat fields, a `translations` map with **no `en` key**, produced once at seed time by `functions/scripts/seed-articles.ts` with a `translationsSourceHash`. Rendered in four places: Home's `HomeNewsCard` (the **first** article by `order` — so `order` is an editorial decision, not just a sort key), Home's `HomeArticlesSection` lane (which **skips index 0** so the news card's article isn't shown twice, then takes 6), `ArticlesRoute` — a pushed list screen with search and category chips — and `ArticleDetailRoute`, a bloc-free screen (hero, blue category pill, headline, read time, body paragraphs). ⚠️ It is the **one detail screen that is stateful**, and only for measurement: a dwell timer plus a `ScrollController` tracking the furthest scroll reached, reported once on `dispose` as `Article Read { seconds, scroll_pct, paragraphs }`. None of that state reaches `build`, so the screen still renders purely from the model the route carries. An article short enough to fit the viewport reports `scroll_pct` **100**, resolved by a post-frame extent check — left at 0 it would be indistinguishable from a bounce. ⚠️ Home's card and lane **hide themselves** on error while `ArticlesPage` **surfaces** it with a retryable `DSStateView.error`. All three stateful consumers own a fresh `sl<ArticlesBloc>()` and close it in `dispose`; Home reads **`all`**, the list screen reads **`visible`**. `body` is a **paragraph array**, so `translateArticleText` enforces a count-and-order guard — a dropped paragraph would silently delete advice. `excerpt` is its own field, not a clipped `body[0]`. `ArticleCategory.fromWire` degrades to **`other`**, a hidden bucket excluded from `filterable`. ⚠️ Needs a composite index on `published ASC, order ASC`.
...
8. UseCases — search/brand, log-event, fetch-product-by-image, cat CRUD, auth, subscription, saved-products (get/isSaved/save/unsave), saved-litters (same four), scan-history, litter-history, get-recipes, health-events (get/add/delete)
9. Services (**7**) — `ScanTrackingService`, `CatTrackingService`, `ReviewPromptService`, `RemoteConfigService`, `SessionReplayService`, `NotificationService`, `UserAnalyticsService`. ⚠️ Registration order in `_registerServices` is load-bearing: `RemoteConfigService` → `SessionReplayService` → `UserAnalyticsService`, because each reads the one before it
10. BLoCs (**18**, registered as factories) — Splash, OnBoarding, Search, Home, Profile, Recipes, FoodGuide, Articles, ProductListing, ProductDetail, LitterDetail, CatListing, CatDetail, CatCreate, Paywall, SavedProducts, ScanHistory, HealthCarnet. ⚠️ **Four** blocs are deliberately absent from `main.dart`'s `MultiBlocProvider`: `CatCreateBloc`, then `FoodGuideBloc` and `ArticlesBloc` (Home's `FoodGuideSection` and `HomeNewsCard` are their only consumers), and now `HealthCarnetBloc` — each is resolved with `sl<XBloc>()` in its consumer's `initState` and closed in `dispose`. A shared `HealthCarnetBloc` would carry one cat's records into the next cat's carnet
...
- **Storage**: Cat profile images, plus `recipes/{id}.jpeg` (recipe photos, public), `products/` (cached product images, shared across all users) and `scans/` (raw user scans, plus `{requestId}-display.jpeg` — a per-user image fallback shown only to the scanner when no web image was found; never promoted to `products/`)
- **Analytics**: Screen view tracking via custom RouteObserver
- **Remote Config**: three keys (`RemoteConfigService`, all **fail-open** — the in-app defaults keep every feature on). `onboarding_scan_enabled` (default `true`) drops the scan + result beats from onboarding with no build. `session_replay_enabled` (default `true`) and `session_replay_sample_percent` (default `100`) are the live levers for Mixpanel Session Replay. 1 h minimum fetch interval, so a console flip takes up to an hour plus a cold launch to land
...
- **Onboarding lifecycle** — Started / Get Started Tapped / Step Viewed / Step Back / Completed (⚠️ `Attribution Selected` + `Skipped` and the `attribution_source` People property are now **unreachable** — phase 2 became `recipesArticles`; the screen is parked, not deleted), plus a separate `Onboarding Scan Captured / Succeeded / Failed / Skipped` namespace for the onboarding scan (Home's scan uses different names for the same user action)
...
- **Localization**: every user-facing string goes through `gen_l10n` — **6 locales** (`en` template, `de`, `es`, `fr`, `hu`, `pt`) at 825 keys each, currently in parity. `l10n.yaml` sets `nullable-getter: false`; `pubspec.yaml` has `generate: true`, and the generated `app_localizations*.dart` are committed under `lib/l10n/`. A new string must be added to **all six** ARB files. Localized *art* goes through `localizedAssetPath(...)`; every call site passes all six locales explicitly, but the helper's own `available` **default** is a stale `{en, es, fr, hu}` — a new call site relying on it would silently serve English art to `de` and `pt`. Details in `docs/design.md` §13
