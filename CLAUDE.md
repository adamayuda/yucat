...
6. Mappers — Brand, Product (multiple variants), Litter (response ↔ entity, entity ↔ model), SearchProduct, Cat (entity ↔ document, entity ↔ model), Recipe (entity → model), FoodGuide (document → entity, entity → model)
...
10. BLoCs (**16**, registered as factories) — Splash, OnBoarding, Search, Home, Profile, Recipes, FoodGuide, ProductListing, ProductDetail, LitterDetail, CatListing, CatDetail, CatCreate, Paywall, SavedProducts, ScanHistory. ⚠️ `FoodGuideBloc` is the **second** bloc deliberately absent from `main.dart`'s `MultiBlocProvider` (after `CatCreateBloc`) — Home's `FoodGuideSection` is its only consumer and owns the instance
...
- **Firestore**: cats live in a **top-level `cats` collection** with a `user` DocumentReference field (not a per-user subcollection); image-scan logs at `/scans/{requestId}`; the recipe catalogue at `/recipes/{slug}` and the food guide at `/foodGuide/{slug}` (both public read, Admin-SDK write only). ⚠️ Rules **and indexes** for this project are managed outside the repo — `firebase.json` has no `firestore` block, so deploying a rules file from here would replace the console ruleset wholesale. The recipes **and food-guide** list queries (`where('published', == true).orderBy('order')`) each **need a composite index** on `published ASC, order ASC`; the `recipes` one exists in `yucat-d8fb5`, and `foodGuide` needs its own, created with `gcloud firestore indexes composite create --collection-group=recipes …`. An equality filter plus an `orderBy` on a different field always needs one — the single-field auto-indexes are not enough
