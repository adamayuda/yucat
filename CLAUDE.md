...
- **Device features**: `camera` + `image_picker` (product-image scan and cat photos); `share_plus` (the health carnet's plain-text share)
...
- **splash**: Bootstraps services and routes to onboarding, paywall or main. Shows the logo face as a Lottie (`splash-face.json`, YUC-31) that pops in over `splashPink`; `SplashBloc` routes no earlier than 1 s (a floor run in parallel with boot, so the pop is never cut). ⚠️ The native launch screens (iOS `LaunchScreen.storyboard`, Android `launch_background.xml`) are **plain pink with no image** on purpose — a static face there would shrink to the animation's first frame and re-pop. iOS caches launch screens: delete and reinstall to see a change
...
- **profile**: Hub screen — Your Cats, then a **Health record** row (household overdue / urgent / soon count from `ProfileLoadedState.health`, a `List<CatHealthSummary>`; one cat pushes `HealthCarnetRoute`, several open `showHealthCatPickerSheet`; hidden when no carnet could be read), Saved Products and Scan History rows with counts, legal links, and two QA rows gated on `kQaToolsEnabled` (debug **and TestFlight**, never the App Store): Reset Onboarding, and **Reset test user** (`QaResetService`) — signs out of RevenueCat, OneSignal, Mixpanel and Firebase, clears prefs and restarts from splash as a brand-new uid. ⚠️ Deleting the app does *not* do this: the anonymous Firebase session survives an uninstall in the iOS Keychain, so a reinstall is the same user, same RevenueCat customer, same sandbox entitlement

**Auth**
- **auth**: Anonymous Firebase authentication

**Cat lifecycle**
- **cat**: Cat profile CRUD (Firestore + Storage). Owns `domain/entities/cat_allergen.dart` — the allergen catalogue (stable keys + English match needles) shared by the carnet, the product assessment and the recipe filter. Also hosts two rules engines in `presentation/utils/` — `cat_diet_recommendations.dart` (owner-facing tips) and `cat_product_recommendations.dart` (per-cat product picks). See *Product Assessment Logic* below
- **cat_create**: Multi-step cat profile creation wizard (see *Core domain* below). Its photo step and Cat Detail share `lib/presentation/components/photo_source_sheet.dart` (`pickPhotoFromSheet`). ⚠️ Profile photos are **downscaled to 1024 px JPEG in `CatDataSource.uploadCatProfileImage`** (both the wizard and the in-place change go through it), and the wizard's `UpdateCatUsecase` deletes the replaced photo after the document write. ⚠️ The health step's **"None" chip is the sentinel `CatCreateModel.noHealthCondition` (`'none'`)** — it must exist because the step won't proceed on an empty selection, but it is a wizard answer, not a condition. Only `persistedHealthConditions` (sentinel stripped) may reach a use case, a document or an analytics property; `CatDocumentMapper` also strips it on read so cats saved before YUC-9 stop showing "1 condition" in the list and "None" on the profile. `CatModelToCreateMapper` re-seeds the sentinel when an edited cat has no conditions
- **cat_listing**: Lists the user's cats. Each card carries a `HealthDuePill` ("Rabies · in 3 days") when the cat's carnet has an item overdue, urgent or due within 30 days — never for `later` or `toSchedule`, so a pill means something. `CatListingBloc` emits the cards first, then a second `CatListingLoadedState` with `health` (a `Map<catId, CatHealthSummary>` from `resolveHouseholdHealth`); a cat whose read failed is simply absent from the map. Tapping the pill opens the carnet directly, not detail
- **cat_detail**: Single-cat detail view. Hosts the entry row into the health carnet — **live** since the visibility work: nearest due item + urgency pill, record count and last weight, from `CatDetailLoadedState.health` (a `CatHealthSummary`, resolved by the bloc *after* the cat emits so the page never waits on Firestore; refreshed by `CatDetailHealthRefreshEvent` on return from the carnet and after a reload). ⚠️ `health == null` means the read failed or hasn't landed and renders the neutral static row — never the setup invite. The hero avatar is **tappable** (YUC-29): it opens the shared `showPhotoSourceSheet` and `CatDetailPhotoChangedEvent` → `UpdateCatPhotoUsecase` uploads, points the document at the new URL, then best-effort deletes the replaced Storage object — in that order, so a failed step never leaves a broken URL. Emits the wizard's `Cat Profile Updated` with `fields_changed: ['profileImage']` plus `source: cat_detail`. ⚠️ The edit pencil is now **awaited** and followed by `CatDetailReloadEvent`, which re-reads the cat so a wizard edit shows on return; the reload swallows every failure and keeps the current model. Cat Listing refetches **unconditionally** on return from detail (and no longer flashes the skeleton over an already-loaded list), so no "changed" flag is threaded through delete / photo / edit
- **health_carnet**: Per-cat veterinary record — vaccines, antiparasitics, visits, labs, surgeries and weight — plus a **derived schedule** of what is due next. Reached by a pushed `HealthCarnetRoute` from Cat Detail. On the **first load of an empty carnet** a four-question setup sheet opens by itself (last vaccine / deworming / vet visit / indoor-or-outdoor, each skippable) and writes the answers as ordinary `done` records through one **sequential** `HealthCarnetSetupCompletedEvent` — never three add events, which would race in `_write`; a prefs flag makes it once per cat and `HealthSetupCard` keeps it reachable. ⚠️ **The schedule is never stored**: `computeDueItems(cat, history, now)` is a pure function of the records plus the profile, so there is no second store to keep in sync. A freeform **treatment** can be a **medication course** (`course_end_at` + `doses_per_day`; `activeCourses` derives what runs today — one row per course on the treatments card, rings on the calendar; not a protocol, never in `computeDueItems`). The app bar's **share icon** hands the carnet to the system share sheet as plain text (`buildHealthShareText`, pure and tested; no PDF). The carnet can also **scan a vaccination-booklet page** (`readHealthBooklet` callable → `health_booklet_review_sheet.dart`, rows confirmed before a sequential write; the photo is never persisted). A record can carry one **photo attachment** (booklet page, lab result) at `cats/{catId}/health/{eventId}.jpeg` in Storage, uploaded *after* the document write and patched in; a failed upload keeps the record (`HealthAttachmentFailed`). Records live at **`cats/{catId}/health_events/{eventId}`** — a subcollection that predates the feature (`cat_datasource.dart` already cascade-deletes it on cat delete) and the app's **first** persisted `Timestamp`. Protocols (FVRCP, rabies, FeLV, deworming, annual check-up, …) are age-banded phase lists sourced from WSAVA/AAFP/ESCCAP; `dental_scaling` has **no phases** on purpose and FIP is deliberately absent. ⚠️ `CatEntity` has no `birthDate`, so one is approximated from `age` — which is why a protocol with **no record** renders as neutral "to schedule" rather than red "overdue", and why items beyond a 12-month horizon are dropped. `CatEntity.lifestyle` gates the outdoor protocols: outdoor cats get the FeLV booster scheduled and adult deworming monthly (an `outdoorIntervalDays` override on the same protocol, not a second one); heartworm stays **manual-add** because it is region-dependent. Each due item carries a **"Learn more"** link to its article (`healthProtocolArticleSlug`, only when that article resolved). The **History tab** shows a monthly weight chart plus a timeline of past acts (swipe-to-delete, which re-derives the schedule); it reads `done` records only, since `snoozed` rows are scheduling residue rather than acts. ⚠️ The chart's y-axis is **zoomed to the data, not anchored at zero**, so every bar carries its actual kg value — don't drop that label. The **Calendar tab** plots every *dated* item — filled dots for recorded acts, rings for scheduled ones — and ⚠️ deliberately omits `toSchedule` items, which are dated today as a placeholder and would otherwise pile onto today's cell. **The vet contact** (`CatEntity.vet`, a `CatVetContact`) is edited here too — one-tap `tel:` call, add-sheet prefill — and stored on the cat document on the same only-write-when-set contract, cleared via `updateCatVet`. **Allergies** are edited here but stored on the **cat document** (`CatEntity.allergies`, `CatAllergen` keys — never free text, so they stay matchable in all six locales); they flag matching products in `cat_product_assessment.dart` and hide matching recipes on the Recipes tab. ⚠️ `CatDocumentMapper.toDocument` writes `allergies` **only when non-empty**, so a cat-wizard save can't wipe the list — which also means it can't clear it, hence the separate `updateCatAllergies`. ⚠️ Security rules for the subcollection must be authored **in the Firebase console** — subcollection rules do not inherit. **Full details in `lib/features/health_carnet/README.md`** — read it before touching the feature

**Product discovery**
- **home**: Dashboard + scan host. ⚠️ Scan failures render `HomeScanErrorView` (one layout per backend `outcome`, up to three exits: scan again / photograph the back label / search by name) when `HomeErrorState.outcome` is set; transport errors keep the plain `DSStateView.error`. The loading theater has a **Cancel** link (`ScanAbandonedEvent` → a generation counter in `HomeBloc` drops the in-flight result; the backend still finishes and caches). `HomeBloc` also owns the back-label rescue (`LabelImageCapturedEvent` → `AnalyzeProductLabelUsecase` → the same success tail as a scan). It is a **discovery surface** with one per-cat exception: `HomeScanHeader` → **`HomeHealthNextUpCard`** (the carnet's surface on the screen every user sees — the single nearest dated act across all cats with a "+N more" household caption, or the setup invitation for a cat with no records, or an **all-clear** naming the next act beyond the 12-month horizon; chosen by the pure `resolveNextUp`; absent only when `HomeLoadedState.healthNextUp` is null, i.e. no cats or every read failed) → `FoodGuideSection` → `HomeRecipesSection` → `HomeArticlesSection` → `HomeMissionCard`, and `HomeDashboardPage` is **stateless**. ⚠️ `HomeBloc` resolves the card through **`resolveHouseholdHealth`** (`health_carnet/presentation/utils/cat_health_summary_resolver.dart`) — the one read path shared with Cat Detail's row and the cat-list pills, backed by `health_events_cache.dart`, a process-global mirror that `HealthCarnetBloc` (the only writer) refreshes on every re-derive — so the read costs Firestore once per cat per session even though Home re-fires on every tab visit and after every scan; a failed read drops that cat, and tapping the card pushes `HealthCarnetRoute` and re-fires Home on return. ⚠️ `HomeGreetingCard` ("Welcome back" + the cat picker), `HomeCatSelector` and — since YUC-24 — `HomeNewsCard` are **parked, not deleted**: unmounted from Home, still in `widgets/`, kept for reuse — nothing imports them today, so don't prune them as dead code. The cat snapshot, completion nudge, diet tips and product picks were removed from Home and now live only on **cat_detail**, and the saved-products preview is gone too (Profile is the only route to `SavedProductsRoute`) — so `HomeLoadedState` carries **`cats` and `healthNextUp`**, and `HomeBloc` no longer depends on `GetSavedProductsUsecase`. ⚠️ `cats` is loaded for the `UserAnalyticsService.syncCats` (and, since the carnet, `syncHealth` — four `health_*` People props, never written from an empty read) People-profile correction on every Home load (the OneSignal `has_cat` tag was dropped 2026-09-12 to fit the Free plan's six-tag budget) and, since the health card, as the input to `healthNextUp`. Nothing renders the list itself. **`HomeScanHeader` is the top of the page** and the only stateful widget in it (one repeating `AnimationController` for the viewfinder sweep): one full-bleed blue slab holding the read-only `SearchTextField` *and* the scan pitch, so the top reads as a single section rather than a pill floating above a banner. ⚠️ It bleeds under the status bar by adding `MediaQuery.padding.top` itself, which is why `HomeDashboardPage`'s `SafeArea` passes **`top: false`** and the `ListView` starts at **zero** top padding — restoring either leaves a `pageBackground` band above the blue. Status-bar icons are handled by nested `AnnotatedRegion`s (header light, page dark), so they flip as it scrolls off. ⚠️ **No barcode iconography anywhere in it** — the backend identifies a *photo of the package* and the barcode flow was deleted from both client and functions, so the affordance is a viewfinder framing a pack, never a barcode glyph. `HomeSkeleton`'s `_ScanHeaderBone` mirrors its height and must move with it — YUC-24 trimmed both together (viewfinder 104→**88** px, inner 72→**62**, the search→pitch gap and the slab's bottom inset `sizeL`→**`sizeS`**), since the viewfinder, not the copy, sets the header's floor. Scanning now has **two** entry points — the header CTA and the nav's Scan slot — separated only by `Scan Started { source }`; `HomePage` still hosts the scan theater (`HomeScanningState` → `HomeLoadingWidget`), which is why the nav slot switches to Home before pushing `ScannerRoute` and the header (already on Home) does not. The **recipe swimlane is live**: `HomeRecipesSection` is stateful and owns a **fresh** `sl<RecipesBloc>()` — deliberately *not* the root-owned instance the Recipes tab reads, because `_onInitial` emits `RecipesLoadingState` unconditionally (a shared bloc would wipe the tab's query + category chip) and `RecipesLoadedState.visible` applies those filters. It reads **`all`**, takes the first 6, and closes its bloc in `dispose` — the one line that must NOT be copied from `RecipesPage`. On error or an empty catalogue the section renders nothing at all, header included; the retryable error state stays on the tab. The **food-guide swimlane is live too**, on the identical pattern (own `sl<FoodGuideBloc>()`, closed in `dispose`, hides itself on error or empty); tapping a tile pushes `FoodGuideDetailRoute`, and its "See all" pushes `FoodGuideRoute`. The **articles lane is live too** (`HomeArticlesSection`, own `sl<ArticlesBloc>()` closed in `dispose`, hides itself on error or empty), showing the first six articles. `HomeMissionCard` closes the page — a flat `tintMist` panel with the mission statement beside `cat-idle.json`, an 80 px looping Lottie mascot (YUC-25 replaced the static `cat-thumb.svg`, which the cat-create coat step still uses); pure ARB chrome, no data source and no tap target by design. ⚠️ It passes **`frameRate: FrameRate.composition`** — the loop never ends and a `ListView` keeps it ticking off-screen inside the cache extent, so the default would repaint at the device refresh rate (4× per authored frame on a 120 Hz screen). The three lanes are added to the page `ListView` **unpadded** so they scroll edge-to-edge. ⚠️ All three content lanes (food guide, recipes, articles) mix in `ContentLaneAnalytics` and emit **`Content Lane Viewed { section, item_count }`** once per appearance — the honest denominator for lane conversion, since a lane that hid itself on an empty or failed read must not count as an ignored impression. The guard lives in each State, not in the bloc: `RecipesBloc`, `FoodGuideBloc` and `ArticlesBloc` are each constructed twice concurrently (the Home lane plus its own screen), so a bloc hook would fire twice per page load. ⚠️ `ContentSection.newsCard` and `ContentSource.homeNewsCard` are now **unreachable** — they existed only for the parked news card; keep them so a re-mount restores the old names
...
- **product_detail**: Product detail with cat-specific assessment; bookmark icon toggles save state via `SavedProductsRepository`. The no-data card (score 0) carries a **"Photograph the back label"** CTA — the scan-pipeline Phase 2 rescue: it unwinds to the tab shell, activates Home and pushes `ScannerRoute(mode: ScanMode.label, labelTarget: …)`; `HomeBloc` owns the resulting theater and pushes the rescued product exactly like a scan (`Product Selected { path: label }`). The **guaranteed-analysis card** (`NutritionGridCard`) has an **as-fed / dry-matter toggle** (`ProductDisplayModel.dryMatterFactor`; moisture always reads as fed since it is the divisor), seven cells (protein, fat, carbs, fiber, moisture, ash, energy) and an energy estimate marked `~` — it is modified-Atwater arithmetic, not a label value. Under the per-cat verdict, **`DailyPortionCard`** shows the selected cat's daily portion from `daily_portion.dart` (pure, tested: `RER = 70 × kg^0.75` × a life-stage / reproduction / weight-goal / neuter ± activity factor, converted to grams of *this* food and, when `packageSize` parses as a single-serve unit ≤ 500 g, to pouches or cans; a treat gets the 10 % treat budget instead; a cat without a weight gets a nudge). The **overflow menu** (`product_overflow_sheet.dart`) is live: share (text + `StoreLinks.current`) and report-a-problem (five fixed reasons → `Product Reported { reason, product_key }`, no free text). Under the verdict, **`BetterAlternativesSection`** lists up to three catalogue foods that out-fit the product for the *selected* cat (`betterAlternativesFor` in `cat_product_recommendations.dart` — per-cat fit first, quality on a tie, the product itself excluded by the `brand__name` identity). It is nested *inside* `CatAssessmentSection` on purpose: the selected cat is that widget's local state, and a sibling would need it lifted just to stay in sync. Hidden while loading and when nothing beats the product — a great food ends at its verdict. Emits `Alternatives Shown` / `Alternative Tapped` from the widget layer (the bloc knows nothing about cats)
...
| `age` | `int?` | Months (0–311; e.g. 30 = 2yr 6mo). Display via `ageGroup` or `_formatAge`. ⚠️ A **snapshot** unless `birthDate` is set — then derived on every read |
| `birthDate` | `DateTime?` | Optional exact birthday from the wizard's Age step (`birth_date`, the app's second `Timestamp`). When set, `age` **and `ageGroup`** are re-derived on read so the profile ages itself; moving the age wheels clears it. Adding a field like this touches ~12 hand-maintained places — table in `lib/features/cat_create/README.md` |
| `weight` | `double?` | Kg. Since the carnet visibility work, **a newest weigh-in logged in the carnet updates it** (`updateCatWeight`, best-effort), so it is no longer only a wizard snapshot; `weightCategory` stays the owner's answer |
| `gender` | `String?` | |
...
| `allergies` | `List<String>?` | `CatAllergen` keys, edited in the carnet; written only when non-empty (see health_carnet) |
| `vet` | `CatVetContact?` | Edited in the carnet; written only when set, cleared via `updateCatVet` |
| `lifestyle` | `String?` | `indoor` / `outdoor` (`CatLifestyle`), asked by the carnet's setup sheet; written only when set, cleared via `updateCatLifestyle`. Outdoor auto-schedules the FeLV booster and switches adult deworming to monthly; heartworm stays manual (no region signal) |

**Product entity** — nutrient fields used by the assessment: `protein`, `fat`, `carbs`, `fiber`, `moisture`, `ash` (`calories` lives on `ProductDisplayModel`, not the domain entity). Plus `name`, `brand`, `score`, `imageUrl`, `pros: List<String>`, `cons: List<String>`, two nullable identity fields added in the scan-pipeline Phase 1 — `cacheKey` (the backend's Algolia objectID, `product.barcode` on the wire) and `gtin` (the pack's EAN-13 when known) — which the saved-products and scan-history codecs deliberately **do not** serialise (they exist for the rescue path and analytics, not for persisted rows), and the V2-era display fields: `isAiIdentified: bool` (true for any image-scanned product — note the `productDetailAiIdentifiedPill` string exists in all six ARBs but has **zero call sites**, so no pill is actually rendered), `format` (display string e.g. "Wet pâté", joined with `packageSize` into `ProductDisplayModel.formatLine` for the hero subtitle), `packageSize` (e.g. "85g pouch"), `description` (2-3 sentence nutrition-focused narrative shown under the verdict headline in `AnalysisCard`), and — mapped since the food-result overhaul — `foodType` (`wet` / `dry` / `treat` / `topper` / `supplement`, null on pre-field rows) and `ingredients: List<String>` (as printed, in order). ⚠️ `ProductDisplayModel.isComplementary` (treat / topper / supplement) drives the hero badge, the note in `AnalysisCard`, and keeps such products **out of** the picks pool and of "Better for {cat}" (a treat gets no alternatives at all) — a 92-point treat is a great treat, not a better dinner. `ingredients` is rendered by `IngredientsCard` (first five emphasised, "Show all", items matching any cat's declared allergen highlighted with the same `CatAllergen` needles the engine uses) and is part of the text the engine scans, which is what made allergen/filler detection depend on the LLM *mentioning* chicken in a pro. Both are on the Algolia records and on the wire; older rows simply have neither.
...
- **Macros *and* calories are normalised to dry-matter basis.** `_Nm.from()` scales protein/fat/carbs/fiber *and* kcal by `100/(100 − moisture)`. Every threshold reads against DMB — which is why a wet food isn't scored as "low protein". ⚠️ Calories used to stay as-fed, which compared a pâté's ~90 kcal/100 g against kibble cutoffs (280–400): no wet food could ever trigger a calorie finding and four of six dimensions were a wet-vs-dry detector. Calorie cutoffs are now kcal/100 g DM (≈ 380 low, 420–440 high; a lean pâté ≈ 340–380, a kibble ≈ 390, a fat-rich food ≈ 500). `ProductDisplayModel.calories` is a **modified-Atwater** (3.5/8.5/3.5) as-fed estimate, not 4/9/4. The as-fed advantage of wet food (water bulks a meal) is an explicit rule — `_satietyMoistureMin` gives overweight/obese cats a pro above 70 % moisture. The kcal finding strings deliberately quote **no numbers**: a DM figure can't be compared to the pack's as-fed label
- **Score = `(70 + weightedDelta).clamp(0, 100)`** — 70 is a neutral baseline, not an average. Weighted delta is `(health×15 + weight×12 + age×10 + activity×8 + neutered×6 + breed×5) ~/ 10`; individual findings contribute ±6 to ±12. ⚠️ **The per-cat score is never displayed.** The product's quality ring is the only number on the detail screen; the cat card shows a **`FitVerdict`** pill (`fit_verdict.dart`: `greatFit` delta ≥ +8 · `goodFit` ≥ 0 · `someCautions` > −10 · `notRecommended`), banded on `delta`, not the clamped score — so neutral is exactly 0, and a **hard flag** (`CatProductFinding.hard`: declared allergen present, kidney + high protein, diabetes + high carbs) forces `notRecommended` regardless of the arithmetic. A second 0–100 ring next to the product's read as a competing grade (and its 70 baseline rendered green); this mirrors litter's "universal score, per-cat notes" model. The numeric score still ranks the "Better for {cat}" picks.
- **One band table for the product score** — `score_bands.dart` (`ScoreBand.of`: Excellent ≥80 · Good ≥60 · Average ≥40 · Poor). `ratingTextForScore` / `ratingColorForScore` are views of it, so the headline and the ring colour can no longer disagree (they used to: label bands 80/60/40 vs colour bands 70/40 put a yellow ring beside "A solid everyday pick"). Green covers Excellent *and* Good.
...
There is a **second rules engine**: `lib/features/cat/presentation/utils/cat_diet_recommendations.dart` (owner-facing diet tips, shown on Home and Cat Detail) — both engines read their dimension weights from **`assessment_weights.dart`** (plus `coat` and `hydration` for tips only), and it keeps one tip per nutrient by priority. Both engines apply the same profile fallbacks (`neutered` bool when `neuteredStatus` is null, `ageGroupFromMonths(age)` when `ageGroup` is null) — the assessment used to skip those dimensions silently. `cat_product_recommendations.dart` alongside it ranks Algolia-fed product picks per cat (`_minBaseScore = 70`; fit floor is `delta ≥ −3` and no hard flag — the old `score ≥ 68` admitted nothing with any con) behind a **process-global cache keyed by cat id** — invalidated on cat edit, but not otherwise refreshed until restart. ⚠️ `test/features/product_detail/cat_product_assessment_test.dart` covers the engine (built on `AppLocalizationsEn()` directly, no widget tree) — extend it when touching a threshold.
...
5. DataSources — `BrandDataSource`, `AlgoliaSearchDataSource`, `RemoteSearchDataSource`, `AuthDataSource`, `CatDataSource`, `CatNarrativeDataSource`, `BrandVerdictDataSource`, `RecipeSeedDataSource`, `HealthEventDataSource`
6. Mappers — Brand, Product (multiple variants), Litter (response ↔ entity, entity ↔ model), SearchProduct, Cat (entity ↔ document, entity ↔ model), Recipe (entity → model), FoodGuide (document → entity, entity → model), Article (document → entity, entity → model), HealthEvent (document ↔ entity — the app's only `Timestamp` boundary)
7. Repositories — Brand, Analytics, Product, Search, Cat, Auth, Subscription, SavedProducts, SavedLitters, ScanHistory, LitterHistory, RecentSearches, CatNarrative, BrandVerdict, Recipes, HealthCarnet
8. UseCases — search/brand, log-event, fetch-product-by-image, cat CRUD, auth, subscription (`HasActiveSubscription`, `GetSubscriptionStatus` → `{isActive, isTrial}`, `LinkSubscriptionUser` → `Purchases.logIn` + `setMixpanelDistinctID` at splash), saved-products (get/isSaved/save/unsave), saved-litters (same four), scan-history, litter-history, get-recipes, health-events (get/add/delete)
8b. Booklet reader — `HealthBookletDataSource` → `HealthBookletRepository` → `ReadHealthBookletUsecase` (the carnet's `readHealthBooklet` callable)
...
10. BLoCs (**18**, registered as factories) — Splash, OnBoarding, Search, Home, Profile, Recipes, FoodGuide, Articles, ProductListing, ProductDetail, LitterDetail, CatListing, CatDetail, CatCreate, Paywall, SavedProducts, ScanHistory, HealthCarnet. ⚠️ **Four** blocs are deliberately absent from `main.dart`'s `MultiBlocProvider`: `CatCreateBloc`, then `FoodGuideBloc` and `ArticlesBloc` (Home's `FoodGuideSection` and `HomeArticlesSection` are their only consumers and own the instances), and `HealthCarnetBloc` — each is resolved with `sl<XBloc>()` in its consumer's `initState` and closed in `dispose`. A shared `HealthCarnetBloc` would carry one cat's records into the next cat's carnet
...
**Empty / error / loading state illustrations use cat mascots, never raster GIFs.** `MascotIllustration` (cat SVG on a tinted circular halo, framed by gently-twinkling stars, with a native bob/twinkle animation) is the single source — `AppLoadingWidget` and both `DSStateView.error()` / `DSStateView.empty()` render through it. Callers pass a full cat figure (`cat-thinking` / `cat-laught` / `cat-rating`) plus a section `tint` that matches the state's mood (e.g. `tintCoral` for errors). A `.json` asset renders as a Lottie inside the same halo (no bob — it animates itself); the scan-failure screen (`HomeScanErrorView`) uses `cat-try-again.json` (YUC-32). The legacy `assets/images/Illustrations/*.gif` were removed.
...
The backend lives at `functions/` (TypeScript, Node 22), co-located with the Flutter app (single-repo, single deploy). It exposes **five Callable Functions** — `fetchProductByImageV2` (product image scan, food **and** cat litter), `analyzeProductLabel` (the back-label rescue: one vision read of the analysis panel, no web search), `readHealthBooklet` (the health carnet's booklet reader: one vision read of a vaccination-booklet page → proposed records, nothing persisted), `generateCatNarrative` and `analyzeBrand` (both onboarding-only, and both degrade to `null` rather than throwing) — plus one **scheduled function**, `nightlySelfHeal`, which re-analyses score-0 rows and backfills missing images off the request path (`SELF_HEAL_DRY_RUN` param for a candidates-only run). Everything model-facing runs on **Claude Haiku 4.5** with forced tool-use for structured output, plus the `web_search_20250305` server-side tool on the analysis path.
...
```bash
# ⚠️ Deploy by NAME — the project also hosts two legacy functions from the yucat-api repo
# that a bare `--only functions` would try to delete (see functions/CLAUDE.md §10).
npx firebase-tools@latest deploy --only functions:fetchProductByImageV2,functions:analyzeProductLabel,functions:readHealthBooklet,functions:nightlySelfHeal,functions:generateCatNarrative,functions:analyzeBrand
npx firebase-tools@latest deploy --only functions:fetchProductByImageV2    # single function
```
...
- **Product & search** — Product Searched / Selected / Detail Viewed / Image Captured / Image Scan Failed, Search Results Viewed, Product Saved / Unsaved, `Push Opened { template_name }` (a tapped push, the only Mixpanel-side attribution for the OneSignal Journeys), `Scan Started { source }` (`ScanSource.homeHeader` / `bottomNav`) — fired when the camera *opens*, which is the only signal separating Home's header CTA from the nav's Scan slot, since `Product Image Captured` fires after the capture and carries no surface, ⚠️ the old Home-surface events are unreachable — `Home Saved Product Tapped`, `Home See All Saved Tapped`, `Home Cat Snapshot Tapped`, `Home Complete Profile Tapped`, `Home See All Cats Tapped` and `Home Active Cat Changed` — the Home revamp removed the surfaces that emitted them; the one live Home event is `Home Health Card Tapped { surface, state: due|setup|all_clear|unknown, protocol_id?, urgency?, days_until?, cat_id }` — historical name, now the carnet's *door* event on every surface (`surface: home|cat_detail|cat_listing`), built only via `healthEntryTapProperties`
...
- **Tests**: `flutter test` runs the unit tests under `test/` — currently the **pure, Flutter-free** parts of the health carnet (`cat_health_schedule.dart` incl. `activeCourses`, `cat_health_summary_resolver.dart`, `health_calendar_layout.dart`, `health_booklet_proposal.dart`, `cat_allergen.dart`, `cat_vet_contact.dart`) plus two label/text builders that use `lookupAppLocalizations` (`health_labels_test.dart`, `health_share_text_test.dart` — the latter needs `initializeDateFormatting`), which is why those files import no Flutter and take `now` as a parameter, plus the per-cat product assessment engine (`test/features/product_detail/cat_product_assessment_test.dart`, built on `AppLocalizationsEn()` directly because the engine returns translated findings). There is no `test` package, so tests import `flutter_test` and must run with `flutter test`, not `dart test`. `integration_test/onboarding_screenshots_test.dart` is the only integration test. Nothing else in the app is covered
- **Localization**: every user-facing string goes through `gen_l10n` — **6 locales** (`en` template, `de`, `es`, `fr`, `hu`, `pt`) at 914 keys each, currently in parity. `l10n.yaml` sets `nullable-getter: false`; `pubspec.yaml` has `generate: true`, and the generated `app_localizations*.dart` are committed under `lib/l10n/`. A new string must be added to **all six** ARB files. Localized *art* goes through `localizedAssetPath(...)`; every call site passes all six locales explicitly, but the helper's own `available` **default** is a stale `{en, es, fr, hu}` — a new call site relying on it would silently serve English art to `de` and `pt`. Details in `docs/design.md` §13
- **Push notifications**: OneSignal (`NotificationService`), **iOS only**, and deliberately no permission prompt at init — the ask lives on the onboarding `reminders` screen (phase 10 of 12). The `notifPrimer` screen before it is a mock that requests nothing — since the carnet visibility work it previews a **vaccine-due reminder**, and the reminders screen's third option reads "Vaccine & vet-visit reminders" under its historical `onboardingRemindersOptionMonthly` key. The service also writes **exactly six tags** (`funnel_stage`, `paywall_seen`, `is_subscriber`, `is_trial`, `last_active_at`, `last_scan_at` — the OneSignal Free-plan cap) at the existing Mixpanel checkpoints, which is what makes drop-off Segments and the two Journeys possible. ⚠️ Free also caps mobile push at 1,000 MAU from 2026-10-01 and the app is already above it. ⚠️ Because permission is asked so late, users who abandon before `reminders` are tagged but **unreachable**. **`docs/onesignal.md` is the source of truth** — read it before touching push or the tags
- **App Store review prompts**: `ReviewPromptService.recordPositiveMoment(trigger:)` gates on **2 positive moments** and 90+ days because Apple caps the native modal at 3 per 365 days. Moments: a scan result with data (asked when the result screen **pops**, never over the push), a product or litter save, a carnet setup or booklet import that wrote records. The counter keeps its historical prefs key `review_scan_count`. ⚠️ The onboarding `rating` screen **no longer** calls `requestReview()` — it spent a modal on every new user before any value; don't reinstate it. TestFlight never shows the modal, so the path is only observable on the App Store
