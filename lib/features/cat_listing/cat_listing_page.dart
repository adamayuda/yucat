...
class _CatListingPageState extends State<CatListingPage> {
  ...
  void _navigateToCreateCat() {
    context.router.push(const CreateCatRoute()).then((_) {
      // Refresh the cat list when returning from create cat page
      _bloc.add(const CatListingFetchCatsEvent());
    });
  }
  ...
  Widget build(BuildContext context) {
    return BlocListener<CatListingBloc, CatListingState>(
      bloc: _bloc,
      listener: (context, state) {
        // List state handling
      },
      child: BlocBuilder<CatListingBloc, CatListingState>(
        bloc: _bloc,
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.pets, size: 80, color: DSColors.darkGrey),
            SizedBox(height: DSDimens.sizeM),
            Text(
              'No cats yet',
              style: Theme.of(
            ...
            ElevatedButton.icon(
              onPressed: _navigateToCreateCat,
              ...
              label: const Text('New Cat Profile'),
            ),
          ],
        ),
      ),
