...
**one thing**: the nearest dated item across every cat (`nearestDueItem` in
`cat_health_schedule.dart`, which excludes `toSchedule` rows because their date is a
placeholder), or the setup invitation for the first cat with no history, or nothing at all.
Nothing is the right answer for a well-kept carnet with nothing due inside a year.
