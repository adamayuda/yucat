...
import 'package:yucat/features/health_carnet/presentation/widgets/health_setup_card.dart';
...
class _HealthCarnetPageState extends State<HealthCarnetPage> {
  ...
  /// The last error tick a SnackBar was shown for, so the listener — which now
  /// also fires on the first load — can tell a failed write from a fresh load.
  int _shownErrorTick = 0;
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<HealthCarnetBloc, HealthCarnetState>(
      ...
      listenWhen: (previous, current) =>
          ...
          (previous is HealthCarnetLoadingState ||
              (previous is HealthCarnetLoadedState &&
                  previous.errorTick != current.errorTick)),
      listener: (context, state) {
        if (state is! HealthCarnetLoadedState) return;
        if (state.errorTick != _shownErrorTick) {
        ...
        _maybeOfferSetup(context, state);
      },
  ...
  Widget _summaryTiles(BuildContext context, HealthCarnetLoadedState state) {
    ...
    return HealthSummaryTiles(
      ...
      todoCount: state.actionableCount,
  ...
  List<Widget> _tabContent(
    ...
    return [
      // Standing invitation while nothing has been recorded — the auto sheet
      // shows once, this is how it stays reachable.
  ...
  /// Asks *when* before recording: the next due date is computed from this
  /// day, so "done today" must not be assumed for an act done last month.
  Future<void> _markDone(BuildContext context, HealthDueItem item) async {
    final now = DateTime.now();
    final picked = await showDatePicker(
      context: context,
      initialDate: now,
      firstDate: DateTime(now.year - 25),
      // Same bound as the add sheet: a record is something that already
      // happened. Booking a future appointment is a different affordance.
      lastDate: now,
    );
    if (!context.mounted || picked == null) return;
    _bloc.add(HealthCarnetMarkDoneEvent(item: item, performedAt: picked));
  }

  String get _setupDismissedKey => 'health_setup_dismissed_${_cat.id}';

  /// First load of an empty carnet → present the setup by itself, once per
  /// cat. The prefs flag is set whatever way the sheet closes, so a "not now"
  /// is honoured on every later visit; the standing card keeps it reachable.
  void _maybeOfferSetup(BuildContext context, HealthCarnetLoadedState state) {
    if (_setupOffered || state.history.isNotEmpty) return;
    _setupOffered = true;
    if (sl<SharedPreferences>().getBool(_setupDismissedKey) ?? false) return;
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!context.mounted) return;
      _openSetupSheet(context, source: 'auto');
    });
  }

  Future<void> _openSetupSheet(
    BuildContext context, {
