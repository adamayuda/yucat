...
///
/// Vets routinely give the 6-month FVRCP booster at 5½ months or book the first
/// annual visit at 11 months. Without this, a dose logged a fortnight before
/// its band opens would leave the band unsatisfied, and the item would go
/// **overdue the day the band opened** — for an act the owner just recorded.
/// A dose before the *first* band of a one-time act (early neutering, a
/// microchip at 6 weeks) counts regardless of lead: the act is done.
const double _kEarlyDoseToleranceMonths = 1.0;

/// The cat's birth date: the real one when the owner gave it, else
/// approximated from the profile's age in months.
///
/// ⚠️ Without `birthDate`, `CatEntity.age` is a snapshot captured once in the
/// create wizard that **never ages**, so the approximation drifts by however
/// long ago the profile was made. Two things keep that survivable:
///
...
///
/// The kitten series is where the approximation hurts, which is why the Age
/// step offers a birthday. Returns null when neither is known.
DateTime? estimatedBirthDate(CatEntity cat, DateTime now) {
  final known = cat.birthDate;
...
/// The single most pressing *dated* item, or null when nothing is.
///
/// `toSchedule` rows are excluded: they carry a placeholder date (today) that
/// stands for "no record yet", not a deadline, and surfacing one on Home as
/// "next up" would read as an instruction. Overdue items have the earliest
/// dates, so "nearest by date" and "most urgent" agree — but this compares
/// dates rather than trusting [computeDueItems]'s order, so Home and the
/// reminder tag (which both read it) cannot drift from each other.
HealthDueItem? nearestDueItem(List<HealthDueItem> items) {
  HealthDueItem? best;
  for (final item in items) {
    if (item.urgency == HealthUrgency.toSchedule) continue;
    final due = item.dueDate;
    if (due == null) continue;
    if (best == null || due.isBefore(best.dueDate!)) best = item;
  }
  return best;
}
...
HealthDueItem? _dueItemFor({
  ...
  if (lastDone?.performedAt != null) {
    ...
    if (interval != null) {
      ...
      // adult schedule.
      // Advance from the band the dose *satisfied*, not from the cat's age at
      // the time — an early dose (see [_earlyDosePhase]) must step past the
      // band it stood in for, or that band would read as missed for ever.
      final from = phase?.fromAgeMonths ?? (ageThen ?? 0);
    ...
    if (birth == null) {
      // With no age at all, only the acts every adult cat needs can be offered.
      // A kitten-only series or the senior panel would be a guess, and a
      // dateless guess reads as an instruction.
      if (!_appliesAtUnknownAge(protocol)) return null;
...
/// The band an early dose stood in for, or null when it was simply a dose in
/// a gap that the next band still has to follow.
///
/// A dose *before the first band* of a one-time act counts whatever the lead:
/// a cat neutered at 3 months does not need neutering at 4. Otherwise a dose
/// counts for the next band only when it landed within
/// [_kEarlyDoseToleranceMonths] of that band opening — close enough that the
/// vet clearly meant it as that dose, not as an extra one.
ProtocolPhase? _earlyDosePhase(HealthProtocol protocol, double ageThen) {
  final upcoming = protocol.nextPhaseFrom(ageThen);
  if (upcoming == null) return null;
  final beforeFirst =
      protocol.phases.isNotEmpty && ageThen < protocol.phases.first.fromAgeMonths;
  if (upcoming.isOneTime && beforeFirst) return upcoming;
  if (upcoming.fromAgeMonths - ageThen <= _kEarlyDoseToleranceMonths) {
    return upcoming;
  }
  return null;
}

/// Whether a protocol can be offered to a cat whose age is unknown: it must
/// apply at 12 months and keep applying for life (its last band open-ended).
/// Rules out the kitten-only FeLV series and the senior panel, keeps the
/// annual check-up even though its *open-ended* band only starts at ten.
bool _appliesAtUnknownAge(HealthProtocol protocol) =>
    protocol.phases.isNotEmpty &&
    protocol.phaseAt(12.0) != null &&
    protocol.phases.last.toAgeMonths == null;
