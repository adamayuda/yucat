          ...
          child: DSCard(
            ...
            child: Column(
              ...
              children: [
                ...
                // ⚠️ Session Replay leaves text readable on purpose (only
                // images are auto-masked), and the SDK masks text *inputs*
                // unconditionally — but not text rendered back afterwards.
                // These three are the only places in the app where a user's
                // own prose about a medical condition, a vet's name or a clinic
                // is shown as plain `Text`, so they are masked by hand. The
                // date and category pill are not PII and stay legible.
                MixpanelMask(
                  child: Text(title, style: DSTextStyles.titleMd),
                ),
                if (event.notes != null) ...[
                  const SizedBox(height: DSDimens.sizeXxxs),
                  MixpanelMask(
                    child: Text(event.notes!, style: DSTextStyles.bodyMd),
                  ),
                ],
