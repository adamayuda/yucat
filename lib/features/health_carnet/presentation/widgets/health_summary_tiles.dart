...
class HealthSummaryTiles extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // ⚠️ `IntrinsicHeight` is load-bearing. This row lives in a `ListView`,
    // whose children get an unbounded height; `stretch` then hands each tile
    // `h=Infinity` and layout throws ("BoxConstraints forces an infinite
    // height") — the whole carnet body went blank. Intrinsic height bounds the
    // row to its tallest tile, so the two still match when one caption wraps.
