/**
 * Seeds the Firestore `foodGuide` collection from scripts/data/food-guide.json.
 *
 * The food guide is a curated catalogue, not something the server discovers at
 * scan time — so like recipes and unlike products, translations are produced
 ...
 * translation.
 *
 * `food-guide.json` holds the canonical ENGLISH copy; this script fills in the
 * other five languages. Adding a category means editing that file and
 * re-running.
 *
 * ⚠️ An empty string in a text field means "this row does not apply" — that is
 * how a dangerous food drops its whyGood/howToServe rows. The prompt is told to
 * return empty fields unchanged; never fill them in here.
 *
 * ⚠️ The `published == true` + `orderBy('order')` query the app runs needs a
 * composite index on `foodGuide` (`published ASC, order ASC`). Indexes for this
 ...
 * and an entry is only re-translated when that hash changes (or with
 ...
 *   npx ts-node scripts/seed-food-guide.ts --dry-run --only=fish
 *
 ...
 *   --limit=N             Only process the first N categories.
 *   --only=<id>           Only process this category id.
 *   --force-retranslate   Re-translate even when the source hash matches.
 *   --prune               Set published:false on stored categories that are no
 *                         longer in food-guide.json. Documents are kept, so a
 *                         retired category can be restored by re-adding it.
 */
...
import {
  FoodGuideItem,
  FoodGuideText,
  canonicalFoodGuideText,
} from "../src/models/food-guide";
import {translateFoodGuideText} from "../src/services/anthropic.service";
...
function sourceHash(text: FoodGuideText): string {
  return crypto
    .createHash("sha1")
...
async function main() {
  ...
  // 1. Gather.
  const raw = JSON.parse(fs.readFileSync(DATA_PATH, "utf8")) as FoodGuideItem[];
  let items = raw;
  if (ONLY) items = items.filter((r) => r.id === ONLY);
  if (LIMIT !== undefined) items = items.slice(0, LIMIT);

  if (items.length === 0) {
    console.error("No food-guide entries matched the filters.");
    process.exit(1);
  }
  console.log(
    `Project ${PROJECT_ID}: seeding ${items.length} entry(ies) into ` +
      `${TARGET_LANGUAGES.length} language(s): ${TARGET_LANGUAGES.join(", ")}`
  );

  // 2. Read what is already stored, so unchanged entries cost nothing.
  const existing = new Map<string, FoodGuideItem>();
  await Promise.all(
    items.map(async (r) => {
      const snap = await db.collection("foodGuide").doc(r.id).get();
      if (snap.exists) existing.set(r.id, snap.data() as FoodGuideItem);
    })
  );

  // 3. Translate.
  const outcomes = await mapPool<FoodGuideItem, SeedOutcome>(
    items,
    CONCURRENCY,
    async (item) => {
      const canonical = canonicalFoodGuideText(item);
      ...
      const translations: Record<string, FoodGuideText> = {};
      ...
      for (const lang of TARGET_LANGUAGES) {
        ...
        const result = await translateFoodGuideText(canonical, lang);
        if (result) {
  ...
  for (const o of outcomes) {
    ...
    console.log(`  ${o.item.id.padEnd(24)} ${bits.join("  |  ")}`);
  }
  ...
  // Orphans: stored entries that food-guide.json no longer lists. Only
  // meaningful on a full run — with --only/--limit everything else looks
  // orphaned.
  let orphans: string[] = [];
  if (!ONLY && LIMIT === undefined) {
    const stored = await db.collection("foodGuide").get();
    ...
    if (orphans.length > 0) {
      console.log(
        `\n${orphans.length} published entry(ies) not in food-guide.json: ` +
          orphans.join(", ")
      );
      console.log(
        PRUNE ?
  ...
  for (let i = 0; i < outcomes.length; i += batchSize) {
    ...
    for (const o of outcomes.slice(i, i + batchSize)) {
      batch.set(
        db.collection("foodGuide").doc(o.item.id),
        {
          ...o.item,
  ...
  if (PRUNE && orphans.length > 0) {
    ...
    for (const id of orphans) {
      batch.set(
        db.collection("foodGuide").doc(id),
        {
          published: false,
          updatedAt: admin.firestore.FieldValue.serverTimestamp(),
        },
    ...
    await batch.commit();
    console.log(`Unpublished ${orphans.length} orphaned entry(ies).`);
  }
...
main().catch((error) => {
  console.error("seed-food-guide failed:", error);
  process.exit(1);
});
