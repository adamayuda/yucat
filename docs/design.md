...
7. ~~**Push notifications**~~ — **Shipped** (this reversed the earlier "deferred" call). OneSignal (`onesignal_flutter`) is wired via `lib/services/notification_service.dart`, **iOS only**, and deliberately does *not* prompt at init — the permission ask lives on the onboarding `reminders` screen. The `notifPrimer` screen before it is a mock and requests nothing.
