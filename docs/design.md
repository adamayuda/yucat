...
| `DSSegmentedControl` | `ds_segmented_control.dart` | Equal-width single-select segments in a `DSRadii.pill` `tintMist` track; the selected segment lifts to `surfaceCard` + `e1`. Borrows its selected/unselected token pair verbatim from `ArticleCategoryStrip._CategoryChip` so the two read as one system — the difference is layout, and it is deliberate: a category strip scrolls and takes any number of chips, a segmented control shows a small fixed set and must never scroll. Past ~4 segments, use the strip. Introduced for the Health Carnet's À venir / Historique / Calendrier switch. |
| `DSTagChip` | `ds_tag_chip.dart` | Compact read-only pill for a piece of profile data — a health condition, an allergy. Tinted fill, `DSRadii.pill`, optional leading icon. Extracted from the private `_ConditionChip` in `cat_detail_page.dart` when the health carnet needed the same shape. ⚠️ Not `DSChip`: that one is the wizard's multi-select control (card radius, selection dot, border, tappable). This is a tag with no affordance — don't swap them. |
...
| `showPhotoSourceSheet` / `pickPhotoFromSheet` | `photo_source_sheet.dart` | Bottom sheet offering *Take a photo* / *Upload from library* (two `DSOptionRow`s), plus a helper that runs the picker and surfaces permission failures as a snackbar. Used by the cat wizard's photo step and the Cat Detail hero (avatar with an `accentInfo` camera badge, progress ring while uploading). |
...
**Home is a discovery surface.** Every section below the search bar is Firestore-backed and owns its own bloc. Its `ListView` runs: read-only `SearchTextField` →
